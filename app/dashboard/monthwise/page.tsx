...
export default function MonthwisePage() {
  ...
  const activeComparative = (cachedComparative?.length ? cachedComparative : comparative)
    ?.find(c => c.type === 'amazon_monthly') ?? null;

  if (!uploadId && !activeComparative) {
    return (
      <main className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center space-y-3">
          <p className="text-gray-500 text-sm">No monthwise data loaded.</p>
  ...
  if (error || !activeComparative) {
    return (
      <main className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center space-y-3">
          <p className="text-red-600 text-sm">{error ?? 'No Amazon monthly comparative data available.'}</p>
          <Link href="/upload" className="text-blue-600 hover:underline text-sm">← Go to Upload</Link>
        </div>
      </main>
    );
    ...
    <main className="min-h-screen bg-gray-50">
      <div className="max-w-[1800px] mx-auto px-4 py-8 space-y-6">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h1 className="text-2xl font-semibold text-gray-900">Amazon Monthly</h1>
