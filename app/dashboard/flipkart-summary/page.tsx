...
export default function FlipkartSummaryPage() {
  const intermediates = useIntermediates();
  const data = intermediates?.flipkartSummary;

  if (!data?.rows?.length) {
    return (
      <main className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center space-y-3">
          <p className="text-gray-500">No Flipkart summary data loaded.</p>
  ...
  const totals = data.rows.reduce(
    (acc, r) => ({ sales: acc.sales + r.sales, cashback: acc.cashback + r.cashback, total: acc.total + r.total }),
    { sales: 0, cashback: 0, total: 0 },
  );
  ...
  return (
    <main className="min-h-screen bg-gray-50">
      <div className="max-w-[900px] mx-auto px-4 py-8 space-y-8">
        <h1 className="text-xl font-semibold text-gray-900">Flipkart Summary Sheet</h1>
          ...
          <table className="min-w-full text-sm border-collapse">
            ...
            <tbody>
              {data.rows.map((r, i) => (
                <tr key={i} className="bg-white hover:bg-gray-50">
                  ...
                  <td className={CELL}>₹{fmtINR(r.sales)}</td>
                  <td className={CELL}>₹{fmtINR(r.cashback)}</td>
            ...
            <tfoot className="bg-blue-50 border-t-2 border-blue-200 font-semibold">
              <tr>
                ...
                <td className="px-3 py-2 text-right tabular-nums text-blue-900">₹{fmtINR(totals.sales)}</td>
                <td className="px-3 py-2 text-right tabular-nums text-blue-900">₹{fmtINR(totals.cashback)}</td>
                ...
                <tbody>
                  {byStateEntries.map(([state, v], i) => (
                    <tr key={i} className="bg-white hover:bg-gray-50">
                      ...
                      <td className={CELL}>₹{fmtINR(v.sales)}</td>
                      <td className={CELL}>₹{fmtINR(v.cashback)}</td>
